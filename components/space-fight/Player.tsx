...
export default class Player {
    ...
    constructor(width: number, height: number, vMax: number, maxX: number, maxY: number, canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) {
        this.width = width;
        this.height = height;
        this.maxX = maxX;
        this.maxY = maxY;
    ...
    draw(x: number, y: number, collideables: { x: number, y: number, width: number, height: number; health: number; takeDamage: (a: { vy: number, vx: number, mass: number; }) => any; }[]) {
        ...
        this.BulletList.forEach(bullet => {
            if (distance([this.x, this.y], [bullet.x, bullet.y]) > this.diagonal ||
